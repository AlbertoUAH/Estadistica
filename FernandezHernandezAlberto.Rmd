...
De forma gráfica, podemos observar que el valor del estadístico T "cae" dentro de la zona de aceptación de la hipótesis nula, por lo que a un 95 % de confianza se puede decir que la diferencia entre los promedios de estatura es cero:
...
$$
\begin{aligned}
\text{Intervalo de confianza (al 80 \%) para la diferencia de medias de estatura: }\left[-0.002; 0.029\right]
\end{aligned}
$$
...
Aunque hemos comprobado que el p-valor es superior al valor $\alpha$ en cada caso, de igual modo podemos realizar el contraste de hipótesis con ambos porcentajes "a mano", planteando $H_0$ y $H_1$:
...
Una vez calculado el valor de T, debemos probar que sea estrictamente menor al valor t-student $t_{m+n-2, \alpha/2}$ . Para ello, creamos una función denominada __contrastar_hipotesis__ que devuelve tanto el valor del estadístico como el valor t-Student:
...
Analizando los resultados obtenidos (coincidiendo el estadístico T con el obtenido en la función _t.test_), en ambos casos $T < t_{m+n+2, \alpha/2}$ (dicho de otro modo, no cae dentro de la zona de rechazo $H_1$) por lo que __no podemos rechazar $H_0$ tanto para un nivel de significación del 5 % como incluso del 20 %. Por tanto, no hay evidencia suficiente para concluir que el promedio de alturas de la primera submuestra sea distinto al de la segunda.__.